...
function topx(x, mode, callback) {
  ...
  consandra.execute(queery, {prepare: true},(err, result) => {
    ...
    callback(topxArray.sortBy(function(o){ return  -o.totalpoints }));
  });
}
...
client.on('guildMemberAdd', member => { 
  ...
  channel('introduceyourself').send({embed}).then(msg => {
    setTimeout(() => {
    ...
    consandra.execute(`SELECT * FROM member_data WHERE userid = '${message.author.id}'`, (_err4, data) => {
      ...
      if (command === 'profile') {
        ...
        if (args[0] != null) {
          ...
          topx(message.guild.memberCount, 'total', () => {
            rank += globalPlsWork.map(function(result) { return result.userid;}).indexOf(mentionedUser.id)
          })
          ...
          topx(message.guild.memberCount, 'total', () => {
            rank += globalPlsWork.map(function(result) { return result.userid;}).indexOf(message.author.id)
          })
      ...
      if (command === 'rules') {
        const embed = new Client.MessageEmbed()
          ...
          .setTitle(`**Rules**`)
          .setDescription("     ")
          // .setThumbnail(message.guild.iconURL('png'))
          .addField("Please do", 'Use the correct channels when sending messages, and think before @ing, or DMing a member')
          .addField("Don't be", 'racist, homophobic, transphobic, antisemitic or generally disrespectful')
          .addField("Don't post", 'NSFW (Not Safe For Work), NSFL (Not Safe For Life), and Lude content. Advertising other discord server is strictly NOT alowed (Please DM a manager josh about server Advertising)')
          .addField("Serious channel", '[This is an opt-in channel use d!serious for more info]\nWhen using the serious channel please be aware that some members may find certain topics triggering. Also remember that this chat is not a replacement for any kind of support you may have in place/need. Always talk to your GP or therapist if you are experiencing issues with your mental health. There is always someone to help you.')
          .addField('Allowed', 'Self promoteion is alowed in the related channels but do not spam, Swearing is alowed so long as it is not too much or directed at someone with the intention of causing offense/upset , ')
        if (message.channel.id === '342823617333166080') {
          embed.addField('\u200B', '**Welcome to**')
          embed.setImage("https://cdn.discordapp.com/attachments/400622421591326720/732977360269344818/lowresdoddlecord.PNG")
        }
        message.channel.send({ embed });
      }
      if (command === 'voice') {
        channel('general-vc').join();
      }
      if (command === 'dis') {
        channel('general-vc').leave();
      }
    });
  });
});
